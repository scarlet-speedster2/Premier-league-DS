...
def train_predict(clf, X_train, y_train, X_test, y_test):
    ...
    train_classifier(clf, X_train, y_encoded)
    
    # Print the results of prediction for both training and testing
    f1, acc, confidence, _ = predict_labels(clf, X_train, y_encoded)
    ...
    f1, acc, confidence, predictions = predict_labels(clf, X_test, y_encoded_test)
...
def predict_next_round(clf, final_path, current_raw_cleaned_path, statistics=False, stat_path=None, first=True):
    ...
    if len(df) > 0:
        ...
        #print(df_to_predict)
        df_to_predict = df_to_predict.drop(df_to_predict[(df_to_predict['HomeTeam'] == 'Nottingham Forest FC') | (df_to_predict['AwayTeam'] == 'Nottingham Forest FC')].index)
        df_to_predict = df_to_predict.drop(df_to_predict[(df_to_predict['HomeTeam'] == 'Bournemouth') | (df_to_predict['AwayTeam'] == 'Bournemouth')].index)
        ...
        df_to_predict = df_to_predict.drop(columns=['prob_' + str(outcome) for outcome in clf_classes])
